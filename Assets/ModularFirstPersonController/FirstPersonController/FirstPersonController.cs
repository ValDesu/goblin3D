...
public class FirstPersonController : MonoBehaviour
{
    ...
    public void OnSpell_E(InputValue value)
    {
        if(value.isPressed) RaycastOnWall();

    }

    public void OnSpell_Enter(InputValue value)
    {
    ...
    private void Update()
    {
        ...
        if(cameraCanMove)
        {
            yaw = transform.localEulerAngles.y + m_Look.x * mouseSensitivity;

            if (!invertCamera)
            {
                pitch -= mouseSensitivity * m_Look.y;
            }
            else
            {
                // Inverted Y
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && m_Sprint && sprintRemaining > 0f && !isSprintCooldown)
            {
    ...
    private void RaycastOnWall()
    {
        ...
        if (Physics.Raycast(origin, direction, out RaycastHit hit, distance))
        {
            ...
            //TODO : bind a button to trigger
            
        }
        else
        {
            Debug.Log("<color=orange>NO RAYCAST HIT</color>");
        }
